...
export const api = {
  ...
  events: (params?: {
    ...
    per_page?: number;
    page?: number;
  }) => apiRequest<ApiCollection<Event>>(apiUrl('/events', params).replace(API_BASE_URL, '')),
  event: (slug: string) => apiRequest<ApiResource<Event>>(`/events/${slug}`),
  places: (params?: { community?: string; per_page?: number; page?: number }) =>
    apiRequest<ApiCollection<Place>>(apiUrl('/places', params).replace(API_BASE_URL, '')),
  ...
  booking: (token: string, reference: string) =>
    apiRequest<ApiResource<BookingDetail>>(`/bookings/${reference}`, { token }),
  createBooking: (
    token: string,
    payload: { eventSlug: string; items: { ticketTierId: string; quantity: number }[] }
  ) =>
  ...
  savedEvents: (token: string) =>
    apiRequest<ApiCollection<BookingCard>>('/me/saved-events', { token }),
};
